...
public:
    ...
    QString print(int a){
        QString str="";
        if(a==1) preOrder(root,str);
        if(a==2) inOrder(root,str);
        if(a==3) postOrder(root,str);
        return str;
    }
    ...
    void postOrder(Node<T, K>* node,QString& s){
    ...
    void preOrder(Node<T, K>* node,QString& s){
        if (node == nullptr) return ;
        s+=QString::number(node->key) + ' ';
        preOrder(node->left,s);
        preOrder(node->right,s);
    }
    void inOrder(Node<T, K>* node,QString& s){
        if (node == nullptr) return ;
        preOrder(node->left,s);
        s+=QString::number(node->key) + ' ';
        preOrder(node->right,s);
